...
class ADNI(Dataset):
    ...
    def __init__(self, 
        ...
        if mode == 'Train':
            subject_tsv = pd.io.parsers.read_csv(os.path.join(splits_dir, 
        ...
        if os.path.isfile(os.path.join(data_dir, f'{self.experiment_name}.pth')):
            ...
            for idx, row in tqdm(subject_tsv.iterrows(), ncols=60, total=subject_tsv.shape[0]): 
                ...
                slices = self.get_prepared_data(participant_id, session_id)
    ...
    def get_prepared_data(self, participant_id, session_id):
